...
from pynq.overlays.stripped import StrippedOverlay

print("imported")

VIDEO_OUT = True

base = StrippedOverlay("stripped.bit", download=True)
#base = BaseOverlay("base.bit", download=True)
gpio = MicroblazeLibrary(base.iop_arduino, ['gpio'])

print("downloaded")

latch_open = gpio.gpio_open(7)
gpio.gpio_set_direction(latch_open, gpio.GPIO_IN)

latch_pwr = gpio.gpio_open(6)
gpio.gpio_set_direction(latch_pwr, gpio.GPIO_OUT)
...
def blink_status(run, cmd_queue):
    try:
        status_led = gpio.gpio_open(5)
        gpio.gpio_set_direction(status_led, gpio.GPIO_OUT)
        gpio.gpio_write(status_led, 0)
        while run.value:
            if cmd_queue.empty():
                gpio.gpio_write(status_led, 1)
                sleep(.1)
                gpio.gpio_write(status_led, 0)
                sleep(.1)
                gpio.gpio_write(status_led, 1)
...
try:
    while not (base.btns_gpio.read() & 0x8):
        ...
        gpio.gpio_write(latch_pwr, time() <= time_latch_off)
...
run.value = False
if 'procthread' in globals() and procthread.is_alive():
    procthread.join()
    del procthread
if 'statusthread' in globals() and statusthread.is_alive():
    statusthread.join()
    del statusthread
