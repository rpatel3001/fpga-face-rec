...
from pynq.overlays.hlsdmaresize import HlsResizeOverlay
import cProfile
import datetime
...
base = HlsResizeOverlay("hlsdmaresize.bit", XRES, YRES, procScale, download=True)
...
def resizeAndScaleFast():
    return cv2.cvtColor(base.resize(), cv2.COLOR_BGR2GRAY)

def resizeAndScale(frame):
    return cv2.cvtColor(cv2.resize(frame, (XRES//procScale, YRES//procScale)), cv2.COLOR_BGR2GRAY)

def processframes(run, frame, faces_flat, procTime, labels, confidences):
    try:
        ...
        while run.value:
            startProc = time()
            gray = resizeAndScale(frame)
            #gray = resizeAndScaleFast()
            ...
            for i, (x, y, w, h) in enumerate(faces_raw[:numFaces]):
                ...
                labels[i] = label if confidence < MAX_CONFIDENCE else -1
                confidences[i] = confidence if confidence < MAX_CONFIDENCE else -1
...
frame_buf = base.in_buffer #Array(ctypes.c_uint8, XRES*YRES*3)
...
frame = frame_buf #np.frombuffer(frame_buf.get_obj(),
#                      dtype=np.uint8).reshape([YRES, XRES, 3])

procthread = Process(target=processframes, args=(run, frame_buf, faces_flat, procTime, labels, confidences))
...
try:
    while not (base.btns_gpio.read() & 0x8):
        fps = str(round(1.0 / fpsTime, 1))
        start = time()
        procfps = str(round(1.0 / (procTime.value), 1))
        ...
        for label, confidence, (x, y, w, h) in zip(labels, confidences, faces_parsed):
            ...
            if not latch_open or time() < time_latch_off:
                if users[label]['access_level'] >= locks[current_location]['access_level']:
                    time_latch_off = time() + .5
                    status_cmds.put((.5, .5))
                img_path = base_dir + "/log_images/{:%Y-%m-%d-%H-%M-%S}".format(datetime.datetime.now()) + "-%s-%s.png"%(users[label]["name"], "allowed" if users[label]['access_level'] >= locks[current_location]['access_level'] else "denied")
                cv2.imwrite(img_path, outframe[y:y+h, x:x+w])
                cur.execute('''INSERT INTO access_log (lock_id, user_id, access_granted, img_path)
                             VALUES (%d, %d, %d, '%s')'''%(current_location, label, 
                             users[label]['access_level'] >= locks[current_location]['access_level'],
                             img_path))
                db.commit()
            if VIDEO_OUT:
...
#            img = resizeAndScaleFast()
#            outframe.fill(255)
#            outframe[0:360,0:640,0] = img[...]
#            outframe[0:360,0:640,1] = img[...]
#            outframe[0:360,0:640,2] = img[...]
            hdmi_out.writeframe(outframe)

        fpsTime = time() - start
        sleep(max(0, 1.0/30 - fpsTime))
except KeyboardInterrupt:
    pass
...
print("FPS: %.2f, PROC: %.2f" %
      (1.0 / fpsTime, 1.0 / procTime.value))
